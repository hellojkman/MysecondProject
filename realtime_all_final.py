...
for one,two in zip(mmaf,mmsi):
    url = f'http://marineweather.nmpnt.go.kr:8001/openWeatherNow.do?serviceKey={key}&'
    ...
    #print(data['result'])
    data2 = data['result']
    #print(data2['recordset'])
    data3 = data2['recordset']
    #print(data3['record'])
    datafinal = data3['record']
    datafinal2 = [datafinal]

    #print(type(datafinal2))
    #print(datafinal2)
    #final = datafinal['DATETIME''MMAF_CODE''MMAF_NM''MMSI_CODE''MMSI_NM''WIND_DIRECT''WIND_SPEED''AIR_TEMPERATURE''HUMIDITY''AIR_PRESSURE']    
    #print(type(final))
    #print(final)
    #final=datafinal['DATETIME']['MMAF_CODE']['MMAF_NM']['MMSI_CODE']['MMSI_NM']['WIND_DIRECT']['WIND_SPEED']['AIR_TEMPERATURE']['HUMIDITY']['AIR_PRESSURE'].T
